...
a validator for it is added only when [Repository Check Policy](../checks/repository-check-policy.md) admits one. The
adopter enforces the gate contracts in its own task configuration and `ci`.
