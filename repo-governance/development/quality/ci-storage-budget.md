...
[Minimal Sufficiency](../../principles/minimal-sufficiency.md), and [Fail Closed](../../principles/fail-closed.md). It
applies to any hosted pipeline that stores artifacts, logs, or caches for a repository. GitHub Actions appears below
...
Whether a machine check is worth adding follows [Repository Check Policy](repository-check-policy.md). Where it is, such
as a check that every upload declares its retention, an adopter enforces it in its own workflow lint or gate.
